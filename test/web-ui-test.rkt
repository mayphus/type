...
(module+ test
  (test-case "builder home has selectors and links to method reference"
    (define html (render-page (req "/") schemas layouts))
    (check-true (regexp-match? #rx"Input Method Builder" html))
    (check-false (regexp-match? #rx"Explore Chinese input methods from history to hands-on interaction" html))
    (check-equal? (match-count #rx"href=\"/type/exhibits/double-pinyin-flypy\"" html) 1)
    (check-false (regexp-match? #rx"href=\"/type/exhibits/double-pinyin-flypy\\?platform=desktop\"" html))
    (check-false (regexp-match? #rx"href=\"/type/exhibits/double-pinyin-flypy\\?platform=mobile\"" html))
    (check-true (regexp-match? #rx"Method" html))
    (check-true (regexp-match? #rx"Layout" html))
    (check-true (regexp-match? #rx"href=\"/type/methods\"" html))
    (check-true (regexp-match? #rx"Browse all methods" html))
    (check-true (regexp-match? #rx"<select class=\"rime-customizer-select\" name=\"schema\" aria-label=\"Input method\"" html))
    (check-true (regexp-match? #rx"<select class=\"rime-customizer-select\" name=\"schema\" aria-label=\"Keyboard layout\"" html))
    (check-false (regexp-match? #rx"this\\.form\\.submit\\(\\)" html))
    (check-true (regexp-match? #rx"loadCustomizer" html))
    (check-true (regexp-match? #rx"history\\.pushState" html))
    (check-true (regexp-match? #rx"<option value=\"double-pinyin-flypy\" selected=\"selected\">Flypy - Double Pinyin</option>" html))
    (check-true (regexp-match? #rx"<option value=\"double-pinyin-flypy-14\"" html))
    (check-false (regexp-match? #rx"rime-customizer-method is-selected" html))
    (check-false (regexp-match? #rx"rime-customizer-layout is-selected" html))
    ...
    (check-true (regexp-match? #rx"/type/schemas/double-pinyin-flypy/desktop-preview.svg" html))
    (check-true (regexp-match? #rx"/type/schemas/double-pinyin-flypy/skin-preview.svg" html))
    (check-false (regexp-match? #rx"/type/schemas/double-pinyin-flypy-14/preview.svg" html))
    (check-false (regexp-match? #rx"/type/schemas/double-pinyin-flypy-14/skin-preview.svg" html))
    ...
    (check-false (regexp-match? #rx"href=\"/type/\">Museum</a>" html))
    ...
    (check-false (regexp-match? #rx"href=\"/type/desktop\"" html))
    (check-false (regexp-match? #rx"htmx.org" html)))

  (test-case "methods page holds large reference catalog"
    (define html (render-methods-page (req "/type/methods") schemas layouts))
    (check-true (regexp-match? #rx"All methods" html))
    (check-true (regexp-match? #rx"href=\"/type\"" html))
    (check-equal? (match-count #rx"href=\"/type/exhibits/double-pinyin-flypy\"" html) 1)
    (check-true (regexp-match? #rx"href=\"/type/exhibits/double-pinyin-flypy-14\"" html))
    (check-true (regexp-match? #rx"href=\"/type/exhibits/cangjie6\"" html))
    (check-equal? (match-count #rx"href=\"/type/exhibits/cangjie6\"" html) 1)
    (check-equal? (match-count #rx"href=\"/type/exhibits/cangjie5\"" html) 1)
    (check-true (regexp-match? #rx"/type/schemas/double-pinyin-flypy-14/preview.svg" html))
    (check-true (regexp-match? #rx"Double Pinyin" html)))
  ...
  (test-case "home supports Traditional Chinese copy"
    ...
    (check-true (regexp-match? #rx"輸入法構建器" html))
  ...
  (test-case "exhibit page shows target previews with related artifact actions"
    (define html (render-exhibit-page (req "/type/exhibits/double-pinyin-flypy") schemas layouts "double-pinyin-flypy"))
    (check-false (regexp-match? #rx"Flypy double pinyin exhibit" html))
    ...
    (check-true (regexp-match? #rx"/type/schemas/double-pinyin-flypy/desktop-preview-dark.svg" html))
  ...
  (test-case "desktop query still shows all target previews"
    (define html (render-exhibit-page (req "/type/exhibits/double-pinyin-flypy?platform=desktop") schemas layouts "double-pinyin-flypy"))
    (check-true (regexp-match? #rx"name=\"artifact\" value=\"rime\"" html))
    (check-true (regexp-match? #rx"name=\"artifact\" value=\"yuanshu\"" html))
    (check-true (regexp-match? #rx"/type/schemas/double-pinyin-flypy/desktop-preview.svg" html))
    (check-true (regexp-match? #rx"/type/schemas/double-pinyin-flypy/skin-preview.svg" html))
  ...
  (test-case "yuanshu-only exhibit omits Rime download"
    (define html (render-exhibit-page (req "/type/exhibits/double-pinyin-flypy-14") schemas layouts "double-pinyin-flypy-14"))
